...
function SectionHeader({ label, locked, onLock, onReroll }) {
  return (
    <div className="flex items-baseline justify-between mb-3 pb-2 border-b border-[#1a1410]/15">
      <h2 className="text-[11px] tracking-[0.25em] uppercase text-[#6B1F2D] font-semibold" style={{ fontFamily: '"IM Fell English SC", "EB Garamond", serif' }}>
        {label}
      </h2>
      <div className="flex gap-1">
        ...
        >
          {locked
            ? <Lock size={13} className="text-[#6B1F2D]" />
            : <LockOpen size={13} className="text-[#1a1410]/40" />}
        </button>
        <button
          onClick={onReroll}
        ...
        >
          <Dices size={13} className="text-[#1a1410]/60" />
        </button>
      </div>
    </div>
  );
